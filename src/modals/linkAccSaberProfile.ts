...
export default {
    ...
    async execute(interaction: ModalSubmitInteraction) {
        try {
            ...
            if (!url.includes('accsaber'))
                throw new ModalSubmissionError(
                    `Le lien entré n\'est pas un lien AccSaber valide`
                )

            const gameLeaderboard = new GameLeaderboard(Leaderboards.AccSaber)
            const playerData =
                        ...
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(
                                `### ${ldIcon ? `<:${ldIconName}:${ldIconId}>` : ''} ${hyperlink(playerData.name, playerData.url)}\n` +
                                    '✅ Votre profil AccSaber a bien été lié avec votre compte Discord\n' +
