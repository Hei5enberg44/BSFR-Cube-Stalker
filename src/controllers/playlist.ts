...
export default class Playlist {
    ...
    static async getPlayed(
        ...
        if (playerScoresFiltered.length === 0)
            throw new PlaylistError(
                "Aucune map correspondant aux critères choisis n'a été trouvée"
            )
        ...
        for (const score of playerScoresFiltered) {
            ...
            if (index < 0) {
                ...
                const song = {
                    hash: score.songHash,
                    songName: songName,
                    difficulties: [{ characteristic: 'Standard', name: diff }]
                }
    ...
    static async getRanked(
        ...
        const maps = await beatsaver.searchRanked(
            leaderboard,
            starsMin,
            starsMax
        )
        ...
        if (maps.length === 0)
            throw new PlaylistError(
                "Aucune map ranked correspondant aux critères choisis n'a été trouvée"
            )
    ...
    static async getSnipe(
        leaderboard: Leaderboards,
        playerId: string,
        playerToSnipeId: string
    ) {
        // Récupération des scores des joueurs
        const gameLeaderboard = new GameLeaderboard(leaderboard)
        const playerToSnipe =
            await gameLeaderboard.requests.getPlayerData(playerToSnipeId)
        const playerScores =
            await gameLeaderboard.requests.getPlayerScores(playerId)
        const playerToSnipeScores =
            await gameLeaderboard.requests.getPlayerScores(playerToSnipeId)
    ...
    static async getClan() {
        ...
        if (!clanMaps || clanMaps.length === 0)
            throw new PlaylistError("Aucune map à conquerir n'a été trouvée")
        ...
        for (const map of clanMaps) {
            ...
            const diff = map.leaderboard.difficulty.difficultyName
                .toLowerCase()
                .replace('expertplus', 'expertPlus')
            ...
            if (index < 0) {
                ...
                const song = {
                    hash: map.leaderboard.song.hash,
                    songName: songName,
                    difficulties: [{ characteristic: 'Standard', name: diff }]
                }
                playlist.songs.push(song)
