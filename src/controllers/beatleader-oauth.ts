...
export class BeatLeaderOAuth {
    public static async getToken(name: string) {
        const token = await OAuthModel.findOne({ where: { name } })
        return token
    }
    ...
    static async sendClanInvitation(playerId: string) {
        ...
        if (!token) {
            Logger.log(
                'BeatLeaderOAuth',
                'ERROR',
                'Token « beatleader_clan_invite » introuvable'
            )
        ...
        if (!req.ok) {
            ...
            if (req.status === 400) {
                throw new BeatLeaderError(
                    `Une erreur est survenue lors de l'envoi de l'invitation à rejoindre le clan BSFR : ${error}`
                )
            } else {
                Logger.log(
                    'BeatLeaderOAuth',
