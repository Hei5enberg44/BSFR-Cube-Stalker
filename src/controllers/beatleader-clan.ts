...
export default class BeatLeaderClan {
    ...
    static listen(client: Client) {
        ...
        Logger.log(
            'ClanWars',
            'INFO',
            'Écoute des captures de maps pour la clan wars BeatLeader'
        )
            ...
            if (!data.equals(pong)) {
                try {
                    ...
                    Logger.log(
                        'ClanWars',
                        'ERROR',
                        'Le traitement du message websocket à échoué'
                    )
                }
            }
        })
        ...
        ws.on('close', () => {
            ...
            Logger.log(
                'ClanWars',
                'WARNING',
                "Le websocket de BeatLeader s'est fermé"
            )
            setTimeout(function () {
                self.listen(client)
            }, 60 * 1000)
    ...
    static async processCapture(data: ClanRanking) {
        ...
        const changes =
            captureData.changes !== null
                ? captureData.changes.filter(
                      (c) =>
                          c.currentCaptorId === bsfrClanId ||
                          c.previousCaptorId === bsfrClanId
                  )
                : null
    ...
    static async getActionMessage(captureData: ClanRankingData) {
        if (captureData.playerAction === GlobalMapEvent.ranked)
            return 'Un nouveau lot de maps a été classé, ce qui a'
    ...
    static async postChangesWithMessage(
        changes: ClanRankingChange[],
        postMessage: string
    ) {
        if (changes.length > 10) {
            for (let i = 0; i < changes.length; i += 10) {
                await this.postChangesWithMessage(
                    changes.slice(i, i + 10),
                    postMessage
                )
            }
        } else {
            ...
            for (const change of changes) {
                const leaderboard =
                    change.leaderboardId !== null
                        ? await beatleader.getMapLeaderboardById(
                              change.leaderboardId
                          )
                        : null
                const currentCaptor =
                    change.currentCaptorId !== null
                        ? await beatleader.getClanById(change.currentCaptorId)
                        : null
                const previousCaptor =
                    change.previousCaptorId !== null
                        ? await beatleader.getClanById(change.previousCaptorId)
                        : null
                const songName = leaderboard?.song.name
                const songDifficulty = leaderboard?.difficulty.difficultyName

                if (!currentCaptor) message += 'effectué une égalité sur '
                else message += 'capturé '
    ...
    static async postChangesWithScore(
        changes: ClanRankingChange[],
        score: ClanRankingScore
    ) {
        ...
        for (const change of changes) {
            ...
            if (!currentCaptor) message += 'a effectué une égalité sur '
            else
            ...
            const gif = change.leaderboardId
            ...
            if (gif) {
                const attachment = new AttachmentBuilder(gif, {
                    name: 'clansmap-change.gif'
                })
                messageOptions = { ...messageOptions, files: [attachment] }
            }

            await clanUpdatesChannel.send({
                ...messageOptions,
    ...
    static async getAnimatedScreenshot(leaderboardId: string) {
        ...
        for (let i = 0; i < totalFrames; i++) {
            ...
            if (screenshot.subarray(100, -100).find((value) => value !== 0))
