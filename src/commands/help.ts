...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            const getHelp = (page: number, itemsPerPage: number, pageCount: number) => {
                try {
                    ...
                    const containerBuilder = new ContainerBuilder()
                        ...
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(bold('Liste des commandes :')),
                        ...
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(`-# Page ${Locales.get(interaction.locale, 'page_info', page, pageCount)}`),
                        )
            ...
            const message = response.resource?.message as Message<boolean>
            const collector = message.createMessageComponentCollector({ componentType: ComponentType.Button, filter: collectorFilter, time: 900_000 })
