...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            const getLeaderboard = async (page: number, itemsPerPage: number) => {
                try {
                    const ld = await leaderboard.getLeaderboard(leaderboardChoice, classement, page, itemsPerPage)
                    ...
                    const containerBuilder = new ContainerBuilder()
                        ...
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(`### ${ldIcon ? `<:${ldIconName}:${ldIconId}> ` : ''} Classement ${classement === 'pp' ? 'PP' : 'Précision'} Serveur`)
                        )
                        ...
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(`-# Page ${Locales.get(interaction.locale, 'page_info', page, ld.pageCount)}`),
                        )
            ...
            const messageResource = response.resource?.message as Message<boolean>
            const collector = messageResource.createMessageComponentCollector({ componentType: ComponentType.Button, filter: collectorFilter, time: 900_000 })
